...
- react-select dropdown did not play nicely with my css. Tried targeting individual components with classNamePrefix for styling with styled components and changing z-index, but no matter what could not get dropdown to display. Only with one style did it work, but it also wiped out all styling on the page. Had to move on for now.
